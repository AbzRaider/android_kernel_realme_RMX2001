...
void oppo_chg_turn_off_charging(struct oppo_chg_chip *chip)
{
	...
	charger_xlog_printk(CHG_LOG_CRTI, "[BATTERY] oppo_chg_turn_off_charging begin  !!\n");
        chip->chg_ops->charging_disable();
        charger_xlog_printk(CHG_LOG_CRTI, "[BATTERY] oppo_chg_turn_off_charging end !!\n");
}
...
void oppo_charger_detect_check(struct oppo_chg_chip *chip)
{
		...
		chg_err(" zhangchao 00 oppo_charger_detect_check\n");
			...
			chg_err(" zhangchao 05 oppo_charger_detect_check\n");
                        oppo_chg_turn_on_charging(chip);
			chg_err(" zhangchao 06 oppo_charger_detect_check\n");
                }
			...
			else 
			{
                if (oppo_vooc_get_fastchg_to_normal() == true || oppo_vooc_get_fastchg_to_warm() == true) {
                        ...
                        charger_resumed = chip->chg_ops->check_charger_resume();
			chg_err(" zhangchao 07 oppo_charger_detect_check\n");
                        oppo_chg_turn_on_charging(chip);
			chg_err(" zhangchao 08 oppo_charger_detect_check\n");
		        }		
            }
        } 
		else 
	    ...
	    chg_err(" zhangchao 09 oppo_charger_detect_check\n");
            if (!chip->mmi_fastchg) {
				chg_err(" zhangchao 10 oppo_charger_detect_check\n");
            }
            oppo_gauge_set_batt_full(false);
	    chg_err(" zhangchao 11 oppo_charger_detect_check\n");
			#ifdef CONFIG_OPPO_CHARGER_MTK
            if (chip->chg_ops->usb_disconnect && charger_flag == 1) {
                   chip->chg_ops->usb_disconnect();
                   charger_flag = 0;
		   chg_err(" zhangchao 12 oppo_charger_detect_check\n");
            }
			#endif
            if (chip->chg_ops->get_charging_enable() == true) {
	            chg_err(" zhangchao 12-13 oppo_charger_detect_check\n");
                    oppo_chg_turn_off_charging(chip);
		    chg_err(" zhangchao 13 oppo_charger_detect_check\n");
            }
            oppo_chg_set_awake(chip, false);
	    chg_err(" zhangchao 14 oppo_charger_detect_check\n");
        }
	chg_err(" zhangchao oppo_charger_detect_check end\n");
}
...
static void oppo_chg_check_status_full(struct oppo_chg_chip *chip)
{
	...
	chg_err(" zhangchao oppo_chg_check_status_full begin\n");
		
		if (oppo_vooc_get_fastchg_ing() == true) {
				return;
		}
		chg_err(" zhangchao 00 oppo_chg_check_status_full\n");

        if (oppo_vooc_get_allow_reading() == false) {
                is_batt_full = 0;
                fastchg_present_wait_count = 0;
		chg_err(" zhangchao 01 oppo_chg_check_status_full\n");
        } else {
                if (((oppo_vooc_get_fastchg_to_normal()== true) || (oppo_vooc_get_fastchg_to_warm() == true))
                        && (fastchg_present_wait_count <= FULL_DELAY_COUNTS)) {
                        chg_err(" zhangchao 02 oppo_chg_check_status_full\n");
                        is_batt_full = 0;
                        ...
                        if (fastchg_present_wait_count == FULL_DELAY_COUNTS && chip->chg_ops->get_charging_enable() == false
							...
							if (chip->ffc_support && chip->ffc_temp_status != FFC_TEMP_STATUS__HIGH
									...
									chg_err(" zhangchao 03 oppo_chg_check_status_full\n");
							} else {
								oppo_chg_turn_on_charging(chip);
								chg_err(" zhangchao 04 oppo_chg_check_status_full\n");
							}
                        }
                } else {
                        is_batt_full = chip->chg_ops->read_full();
                        fastchg_present_wait_count = 0;
			chg_err(" zhangchao 05 oppo_chg_check_status_full\n");
                }
        }
	chg_err(" zhangchao 06 oppo_chg_check_status_full\n");

		if (oppo_chg_check_ffc_status(chip) == false)
				return ;
		chg_err(" zhangchao 07 oppo_chg_check_status_full\n");
		...
		chg_err(" zhangchao 08 oppo_chg_check_status_full\n");
			...
			chg_err(" zhangchao 09 oppo_chg_check_status_full\n");
                }
				
        } else if (chip->charging_state == CHARGING_STATUS_FAIL) {
                oppo_chg_fail_action(chip);
		chg_err(" zhangchao 10 oppo_chg_check_status_full\n");
        } else {
                chip->charging_state = CHARGING_STATUS_CCCV;
		chg_err(" zhangchao 11 oppo_chg_check_status_full\n");
        }
	chg_err(" zhangchao oppo_chg_check_status_full end\n");
...
static void oppo_chg_pd_config(struct oppo_chg_chip *chip)
{
	...
	if (pd_type == false && chip->chg_ops->oppo_chg_get_pd_type() == true) {
		pd_type = true;
...
static void oppo_chg_update_work(struct work_struct *work)
{
		...
		chg_err(" zhangchao begin\n");
		charger_xlog_printk(CHG_LOG_CRTI, " zhangchao begin\n");
        oppo_charger_detect_check(chip);
	chg_err(" zhangchao test 01\n");

        oppo_chg_get_battery_data(chip);
	chg_err(" zhangchao test 02\n");

        if (chip->charger_exist) {
		chg_err(" zhangchao test 03\n");
                oppo_chg_check_ffc_temp_status(chip);
		chg_err(" zhangchao test 04\n");
                oppo_chg_check_status_full(chip);
		chg_err(" zhangchao test 05\n");
		oppo_chg_battery_notify_check(chip);
		chg_err(" zhangchao test 06\n");
                oppo_chg_get_chargerid_voltage(chip);
		chg_err(" zhangchao test 07\n");
                oppo_chg_fast_switch_check(chip);
		chg_err(" zhangchao test 08\n");
                oppo_chg_chargerid_switch_check(chip);
		chg_err(" zhangchao test 09\n");
        }
        oppo_chg_pd_config(chip);
	chg_err(" zhangchao test 10\n");


        /* oppo_chg_short_c_battery_check(chip); */
        wake_up_process(chip->shortc_thread);
	chg_err(" zhangchao test 11\n");

        oppo_chg_battery_update_status(chip);
	chg_err(" zhangchao test 12\n");

        oppo_chg_kpoc_power_off_check(chip);
	chg_err(" zhangchao test 13\n");

	oppo_chg_print_log(chip);        //zhangchao add


        /* run again after interval */
        schedule_delayed_work(&chip->update_work, OPPO_CHG_UPDATE_INTERVAL);
		chg_err(" zhangchao end\n");
		charger_xlog_printk(CHG_LOG_CRTI, " zhangchao end\n");
}
...
bool oppo_chg_wake_update_work(void)
{
	...
	//cancel_delayed_work(&g_charger_chip->update_work);   //zhangchao add
	chg_err(" zhangchao test 14\n");
	chg_err(" zhangchao test 15\n");
	//zhangchao add
        shedule_work = mod_delayed_work(system_wq, &g_charger_chip->update_work, 0);
	//schedule_delayed_work(&g_charger_chip->update_work, 0);    //zhangchao add
	chg_err(" zhangchao test 16\n");
