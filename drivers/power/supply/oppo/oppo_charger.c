...
static void oppo_chg_set_input_current_limit(struct oppo_chg_chip *chip)
{
	...
	if ((chip->chg_ctrl_by_lcd) && (chip->led_on)) {
		...
		} else if (chip->led_temp_status == LED_TEMP_STATUS__LIMIT_HIGH) {
			if (current_limit > chip->limits.input_current_led_ma_limit_high)
				current_limit = chip->limits.input_current_led_ma_limit_high;
...
static int fb_notifier_callback(struct notifier_block *nb, unsigned long event, void *data)
{
		...
		pr_err("%s: zc cool_down = %d\n", __func__, g_charger_chip->cool_down);
        return 0;
}
#endif /* CONFIG_DRM_MSM */
...
static void oppo_chg_check_tled_status(struct oppo_chg_chip *chip)
{
	...
	static int limit_high_counts =0, high_counts = 0, warm_counts = 0, normal_counts = 0;

	if (chip->temperature > chip->limits.led_high_bat_decidegc_antishake) {		/* >44C */
		high_counts ++;
	...
	} else if (chip->temperature > chip->limits.led_limit_high_bat_decidegc_antishake) {	/* >37C && <= 44 */
		limit_high_counts ++;
		if (limit_high_counts >= TLED_CHANGE_COUNTS) {
			high_counts = 0;
			warm_counts = 0;
			normal_counts = 0;
			limit_high_counts = 0;
			tled_status = LED_TEMP_STATUS__LIMIT_HIGH;
		}
	...
	if (tled_status != chip->led_temp_status) {
		...
		chip->limits.led_limit_high_bat_decidegc_antishake = chip->limits.led_limit_high_bat_decidegc;
		if (tled_status > chip->led_temp_status && tled_status == LED_TEMP_STATUS__WARM)
		...
		else if (tled_status > chip->led_temp_status && tled_status == LED_TEMP_STATUS__LIMIT_HIGH)
			chip->limits.led_limit_high_bat_decidegc_antishake = chip->limits.led_limit_high_bat_decidegc - TLED_HYSTERISIS_DECIDEGC;
			...
			chip->limits.led_high_bat_decidegc_antishake = chip->limits.led_high_bat_decidegc + TLED_HYSTERISIS_DECIDEGC;
		chg_debug("tled status change, [%d %d %d %d %d]\n", tled_status, chip->led_temp_status,
			chip->limits.led_warm_bat_decidegc_antishake, chip->limits.led_high_bat_decidegc_antishake, chip->limits.led_limit_high_bat_decidegc_antishake);
...
/*Benshan.Cheng@ODM_HQ.BSP.TP.Function, 2019/11/22 add for tp charge mode*/
	...
	else if (chip->temperature > chip->limits.led_limit_high_bat_decidegc)
		chip->led_temp_status = LED_TEMP_STATUS__LIMIT_HIGH;
...
static void oppo_chg_protection_check(struct oppo_chg_chip *chip)
{
        ...
        /*wangtao@ODM.HQ.BSP.CHG 2020/01/04 change over vbat charging status*/
                ...
                if ((chip->stop_voter & CHG_STOP_VOTER__VBAT_TOO_HIGH) == CHG_STOP_VOTER__VBAT_TOO_HIGH) {
                        ...
                        oppo_chg_voter_charging_start(chip, CHG_STOP_VOTER__VBAT_TOO_HIGH);
                }
        }
#endif
