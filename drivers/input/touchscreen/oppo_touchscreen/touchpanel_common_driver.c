...
OUT:
	...
	gesture_flag = ts->gesture_test.gesture_backup;
...
static char apk_water_read(struct touchpanel_data *ts)
{
    if (ts->apk_op->apk_water_get) {
        if (ts->apk_op->apk_water_get(ts->chip_data)) {
            return '1';
        }
        return '0';
...
static void apk_water_switch(struct touchpanel_data *ts, char on_off)
{
    if (ts->apk_op->apk_water_set) {
        if (on_off == '1') {
            ts->apk_op->apk_water_set(ts->chip_data, true);
        } else if (on_off == '0') {
            ts->apk_op->apk_water_set(ts->chip_data, false);
        }
    }
}
...
static int init_touchpanel_proc(struct touchpanel_data *ts)
{
    ...
    if (ts->edge_limit_support || ts->fw_edge_limit_support) {
        prEntry_tmp = proc_create_data("oppo_tp_limit_enable", 0666, prEntry_tp, &proc_limit_control_ops, ts);
        if (prEntry_tmp == NULL) {
            ret = -ENOMEM;
            TPD_INFO("%s: Couldn't create proc entry, %d\n", __func__, __LINE__);
        }
...
static void init_parse_dts(struct device *dev, struct touchpanel_data *ts)
{
	...
	TPD_INFO("%s : reset_gpio = %d\n",__func__, ts->hw_res.reset_gpio);
