...
#define TPD_INFO(a, arg...)  pr_err("[nvt]"TPD_DEVICE ": " a, ##arg)
...
void nvt_bootloader_reset_noflash(struct chip_data_nt36672c *chip_info)
{
    ...
    nvt_write_addr(chip_info->s_client, SPI_READ_FAST, 0x69);
}
...
static fw_check_state nvt_fw_check(void *chip_data, struct resolution_info *resolution_info, struct panel_info *panel_data)
{
   ...
   //             }
   				...
   				strlcpy(panel_data->manufacture_info.version, tmp_version,  panel_data->version_len+3);
            } else {
...
//#else
    { .compatible = "tchip,tp",/*TPD_DEVICE*/ },
//#endif
    { },
};
